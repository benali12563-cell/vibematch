...
export default function SwipeCardView({ vendor, imgIdx, setImgIdx, actions, showInfo = true }: Props) {
  ...
  return (
    <div style={{ height: "100%", position: "relative", userSelect: "none", overflow: "hidden", borderRadius: 0 }}>
      {/* Image */}
      <div
        ref={imgRef}
      ...
      <div onClick={() => setImgIdx((i) => Math.max(0, i - 1))} style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "35%", cursor: "pointer", zIndex: 4 }} />
      <div onClick={() => setImgIdx((i) => Math.min(imgs.length - 1, i + 1))} style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: "35%", cursor: "pointer", zIndex: 4 }} />
