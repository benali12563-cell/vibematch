    ...
    <div style={{ minHeight: "100dvh", background: "#000", fontFamily: "inherit", direction: dir }}>
      ...
      <div style={{ padding: "48px 0 0" }}>
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
          {(["preview", "edit"] as const).map((tb) => (
            <button key={tb} onClick={() => setTab(tb)} style={{ flex: 1, padding: "12px 0", background: "none", border: "none", color: tab === tb ? "#00CED1" : "#555", fontSize: 14, fontWeight: 600, cursor: "pointer", borderBottom: tab === tb ? "2px solid #00CED1" : "2px solid transparent" }}>
              {tb === "preview" ? t.preview : t.editProfile}
            </button>
          ))}
        </div>
